...
<div class="user-table-card">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div class="user-controls" style="display:flex; align-items:center; gap:8px;">
            <form method="GET" action="{{ route('jastiper.pesanan.index') }}" style="display:flex; gap:8px; align-items:center;">
                <input name="q" value="{{ $q ?? '' }}" class="user-search-input" type="text" placeholder="Cari berdasarkan ID / No HP / Alamat" style="padding:8px 12px; border:1px solid #DDE0E3; border-radius:8px;">
        ...
        <a href="{{ route('jastiper.pesanan.create') }}" class="btn-add-user" style="padding:8px 14px; background:#2b6be6; color:white; border-radius:8px;">
            + Tambah Pesanan
        </a>
    </div>
        ...
        <table id="pesanansTable" class="table table-custom">
            ...
            <tbody>
                ...
                <tr data-id="{{ $p->id }}">
                    ...
                    <td><span class="truncate" title="{{ $p->alamat_pengiriman }}">{{ $p->alamat_pengiriman ?? '-' }}</span></td>
                    <td>{{ $p->no_hp ?? '-' }}</td>
                    <td class="col-actions" style="text-align:right;">
                        <div class="table-actions">
                            ...
                            <form action="{{ route('jastiper.pesanan.destroy', $p) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-action del" onclick="return confirm('Hapus pesanan #{{ $p->id }}?')">
